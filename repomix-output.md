...
## File: src/app/shared/utils/display-name.validator.ts
````typescript
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-4">
              <h2 class="text-gray-900 dark:text-white font-semibold text-lg">
                {{ 'QUIZ.result_review_title' | translate }}
              </h2>
                    ...
                    @for (option of q.options; track $index) {
                      ...
                      >
                        ...
                        @if ($index === q.correctIndex) {
                          <span class="ml-auto text-xs">{{ 'QUIZ.result_correct' | translate }}</span>
                        }
...
# Playwright reports & test artifacts
/playwright-report/
/test-results/
.mcp.json
.claude/
...
<div class="flex items-end justify-center gap-3 px-4 py-6">
  <div class="flex flex-col items-center gap-2 flex-1 max-w-[120px]">
    @if (second()) {
      <div class="relative w-12 h-12 rounded-full ring-2 ring-gray-300 dark:ring-gray-500 bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-sm font-bold text-gray-700 dark:text-gray-300 overflow-hidden">
        @let secondAvatar = second()!.avatarUrl;
        @if (secondAvatar) {
          <img [ngSrc]="secondAvatar" fill sizes="48px" [alt]="second()!.displayName" class="object-cover" />
        } @else {
  ...
  <div class="flex flex-col items-center gap-2 flex-1 max-w-[120px]">
    @if (first()) {
      <div class="relative w-14 h-14 rounded-full ring-4 ring-yellow-400 dark:ring-yellow-500 bg-yellow-100 dark:bg-yellow-900/40 flex items-center justify-center text-base font-bold text-yellow-700 dark:text-yellow-300 overflow-hidden">
        @let firstAvatar = first()!.avatarUrl;
        @if (firstAvatar) {
          <img [ngSrc]="firstAvatar" fill sizes="56px" [alt]="first()!.displayName" class="object-cover" />
        } @else {
  ...
  <div class="flex flex-col items-center gap-2 flex-1 max-w-[120px]">
    @if (third()) {
      <div class="relative w-12 h-12 rounded-full ring-2 ring-amber-500 dark:ring-amber-600 bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center text-sm font-bold text-amber-700 dark:text-amber-300 overflow-hidden">
        @let thirdAvatar = third()!.avatarUrl;
        @if (thirdAvatar) {
          <img [ngSrc]="thirdAvatar" fill sizes="48px" [alt]="third()!.displayName" class="object-cover" />
        } @else {
            ...
            <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4" novalidate>
              <fieldset class="border-0 p-0 m-0">
                ...
                <hlm-field>
                  ...
                  <hlm-field-error validator="unsafeChars">{{ 'SECURITY.invalid_display_name' | translate }}</hlm-field-error>
                </hlm-field>
                <hlm-field>
...
# Exclude vendored/boilerplate-heavy files and CRUD form components from duplication detection
# CRUD forms (create/edit pairs) inherently share FormGroup structure by design
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-5" novalidate>
        ...
        <div>
          ...
          @if (form.controls.coverUrl.value) {
            <div class="relative mb-2 rounded-xl overflow-hidden h-28 bg-gray-100 dark:bg-gray-700">
              <img
                [ngSrc]="form.controls.coverUrl.value"
                fill
                sizes="500px"
                alt="Cover preview"
                class="object-cover"
        ...
        <div class="space-y-3">
          <button type="button" hlmBtn variant="ghost" (click)="toggleFirstEvent()">
            {{ (showFirstEvent() ? 'CREATE_CLUB.remove_first_meeting' : 'CREATE_CLUB.add_first_meeting') | translate }}
          </button>
          @if (showFirstEvent()) {
            <hlm-field>
    ...
    <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-5" novalidate>
      <div>
        ...
        <input hlmInput id="title" type="text" formControlName="title" class="w-full"
               data-testid="event-title-input" [placeholder]="'CREATE_EVENT.title_placeholder' | translate" />
        @if (form.controls.title.touched && form.controls.title.errors?.['required']) {
          <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ 'CREATE_EVENT.title_required' | translate }}</p>
        }
      </div>
      <div>
        <label for="bookTitle" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ 'CREATE_EVENT.book_title_label' | translate }}</label>
        ...
        @if (isFetchingCover()) {
          <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ 'CREATE_EVENT.book_cover_searching' | translate }}</p>
        } @else if (coverFetchFailed()) {
          <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ 'CREATE_EVENT.book_cover_not_found' | translate }}</p>
        } @else {
          <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ 'CREATE_EVENT.book_title_hint' | translate }}</p>
        }
      </div>
      <div>
      ...
      <div>
        ...
        <app-address-autocomplete
          ...
          [placeholder]="'CREATE_EVENT.address_placeholder' | translate"
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input hlmInput id="theme" type="text" formControlName="theme" class="w-full" [placeholder]="'CREATE_EVENT.theme_placeholder' | translate" />
        </div>
      </div>
      <div>
      ...
      @if (activeQuizzes().length > 0) {
        <div>
          <label for="quizId" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {{ 'CREATE_EVENT.linked_quiz_label' | translate }}
          </label>
          <select id="quizId" formControlName="quizId"
            ...
            <option [ngValue]="null">{{ 'CREATE_EVENT.no_quiz' | translate }}</option>
        ...
        <div class="flex-1 min-w-0 flex flex-col gap-8 order-1 lg:order-2">
          ...
          @if (actionError()) {
            <div class="flex items-start gap-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-4 py-3 text-sm text-red-700 dark:text-red-400" role="alert">
              ...
              <button
                type="button"
                class="ml-2 -mr-1 -my-1 inline-flex h-7 w-7 items-center justify-center rounded-md text-red-700/70 hover:text-red-700 hover:bg-red-100 dark:text-red-400/70 dark:hover:text-red-400 dark:hover:bg-red-900/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500"
          ...
          @if (isMember()) {
            <button hlmBtn variant="outline" (click)="openClubChat()">{{ 'CHAT.open' | translate }}</button>
          }
...
````json
{
  ...
  "CREATE_CLUB": {
    ...
    "cover_url_invalid": "Enter a valid URL (must start with https://)",
    ...
    "submit": "Create club",
    "submitting": "Creating…",
    "add_first_meeting": "Add first meeting",
    "remove_first_meeting": "Remove first meeting"
  },
  "CLUBS": {
    "active": "Active",
    "all": "All Clubs",
    ...
    "empty_title": "No clubs yet",
    "empty_desc": "No clubs have been created yet. Check back soon!",
    "empty_search_title": "No matches found",
    "empty_search_desc": "No clubs match your search. Try different keywords.",
    "my_clubs_empty_desc": "Join a club to see it here."
  },
  "CLUB_DETAIL": {
    "about": "About",
    "back": "Back to clubs",
    ...
    "not_found_desc": "This club could not be found.",
    "load_failed": "Failed to load club details.",
  ...
  "PROFILE": {
    ...
    "social_placeholder_at": "username (without @)",
    "social_placeholder_url": "username or full URL"
  },
  "RANDOMIZER": {
  ...
  "CREATE_EVENT": {
    ...
    "title_label": "Title",
    "title_placeholder": "April Discussion",
    "title_required": "Title is required.",
    "book_title_label": "📖 Book title",
    "book_title_placeholder": "The Master and Margarita",
    "address_placeholder": "Start typing an address…",
    "theme_placeholder": "sci-fi",
    "book_title_hint": "What book will you discuss at this event?",
    "book_cover_searching": "🔍 Searching for book cover…",
    "book_cover_not_found": "No cover found — you can add one manually below.",
    "description_label": "Description",
    "description_placeholder": "What will you be reading or discussing?",
    ...
    "theme_label": "Theme",
    "tags_label": "Tags",
    "tags_placeholder": "fiction, dystopia, classic (comma-separated)",
    "cover_label": "Cover image",
    "linked_quiz_label": "🧠 Linked quiz",
    "no_quiz": "— No quiz —",
    "after_venue_add": "+ Add after-meeting venue",
    "after_venue_remove": "− Remove after-meeting venue",
    "after_venue_name_label": "Venue name",
    "after_venue_address_label": "Address",
    "after_venue_notes_label": "Notes",
    "cancel": "Cancel",
    "submit": "Create Event",
    "submitting": "Creating…"
  },
  "QUIZ": {
    ...
    "create_title_min": "Title must be at least 3 characters.",
    "create_title_max": "Must not exceed 100 characters.",
    "create_desc_label": "Description",
    "create_desc_placeholder": "A brief description of the quiz…",
    "create_desc_max": "Must not exceed 500 characters.",
    "create_continue": "Continue →",
    "create_first_question": "Add your first question",
    "create_another_question": "Add another question",
    "create_question_label": "Question",
    "create_question_placeholder": "What is the main theme of chapter 3?",
    "create_question_required": "Question is required.",
    "create_question_min": "Must be at least 5 characters.",
  ...
  "SECURITY": {
    "invalid_display_name": "Display name can contain only letters, digits, spaces and . ' - _ (2–50 characters)."
  },
  "ERRORS": {
  ...
  "META": {
    "title": "Book Club — Book Clubs of Ukraine",
    "description": "Join book clubs in your city. Book discussions, reader meetups, interest-based communities.",
    "ogTitle": "Book Club — Book Clubs of Ukraine",
    "ogDescription": "Join book clubs in your city.",
    "twitterTitle": "Book Club — Book Clubs of Ukraine",
    "twitterDescription": "Join book clubs in your city. Book discussions, reader meetups."
  },
  "COVER_UPLOAD": {
    ...
    "enter_url": "🔗 Enter URL",
    "hide_url": "🔗 Hide URL",
    "upload_failed": "Upload failed. Please try again or use a URL."
  },
  "SEO": {
...
````json
{
  "AUTH": {
    ...
    "error_invalid_credentials": "Невірний email або пароль."
  },
  "CREATE_CLUB": {
    ...
    "cover_url_invalid": "Введіть коректний URL (починається з https://)",
    ...
    "add_first_meeting": "Додати першу зустріч",
    "remove_first_meeting": "Прибрати першу зустріч"
  },
  "CLUBS": {
    "active": "Активний",
    "all": "Всі клуби",
    ...
    "empty_title": "Клубів ще немає",
    "empty_desc": "Поки що не створено жодного клубу. Зазирніть пізніше!",
    "empty_search_title": "Нічого не знайдено",
    "empty_search_desc": "Жоден клуб не відповідає вашому пошуку. Спробуйте інші ключові слова.",
    "my_clubs_empty_desc": "Приєднайтесь до клубу, щоб побачити його тут."
  },
  "CLUB_DETAIL": {
    "about": "Про клуб",
    ...
    "not_found_desc": "Цей клуб не вдалося знайти.",
    "load_failed": "Не вдалося завантажити деталі клубу.",
  ...
  "NOT_FOUND": {
    ...
    "message": "Сторінка, яку ви шукали, не існує або була переміщена.",
    "backHome": "Повернутися до клубів"
  },
  "MEMBERS": {
  ...
  "PROFILE": {
    ...
    "role_changed": "Роль успішно змінено!",
    "social_placeholder_at": "username (без @)",
    "social_placeholder_url": "username або повний URL"
  },
  "RANDOMIZER": {
  ...
  "CREATE_EVENT": {
    ...
    "title_label": "Назва",
    "title_placeholder": "Квітнева дискусія",
    "title_required": "Назва є обов'язковою.",
    "book_title_label": "📖 Назва книги",
    "book_title_placeholder": "Майстер і Маргарита",
    "address_placeholder": "Почніть вводити адресу…",
    "theme_placeholder": "наукова фантастика",
    "book_title_hint": "Яку книгу обговорюватиметесь на події?",
    "book_cover_searching": "🔍 Шукаємо обкладинку…",
    "book_cover_not_found": "Обкладинку не знайдено — можна додати вручну.",
    "description_label": "Опис",
    "description_placeholder": "Що будете читати або обговорювати?",
    "date_label": "Дата та час",
    "date_required": "Дата є обов'язковою.",
    "location_label": "Місце",
    "location_required": "Місце є обов'язковим.",
    "duration_label": "Тривалість (хв)",
    "theme_label": "Тема",
    "tags_label": "Теги",
    "tags_placeholder": "фентезі, класика, антиутопія (через кому)",
    "cover_label": "Обкладинка",
    "linked_quiz_label": "🧠 Прив'язати квіз",
    "no_quiz": "— Без квізу —",
    "after_venue_add": "+ Додати місце після зустрічі",
    "after_venue_remove": "− Прибрати місце після зустрічі",
    "after_venue_name_label": "Назва місця",
    "after_venue_address_label": "Адреса місця",
    "after_venue_notes_label": "Примітки",
    "cancel": "Скасувати",
  ...
  "QUIZ": {
    ...
    "create_title_min": "Мінімум 3 символи.",
    "create_title_max": "Не більше 100 символів.",
    "create_desc_label": "Опис",
    "create_desc_placeholder": "Короткий опис квізу…",
    "create_desc_max": "Не більше 500 символів.",
    "create_continue": "Далі →",
    "create_first_question": "Додайте перше запитання",
    "create_another_question": "Додати ще одне запитання",
    "create_question_label": "Запитання",
    "create_question_placeholder": "Яка головна тема розділу 3?",
    "create_question_required": "Запитання є обов'язковим.",
    "create_question_min": "Мінімум 5 символів.",
  ...
  "SECURITY": {
    "invalid_display_name": "Ім'я може містити лише літери, цифри, пробіли та . ' - _ (2–50 символів)."
  },
  "ERRORS": {
    "timeout": "Сервер занадто довго не відповідає. Спробуйте ще раз.",
    "serverError": "Сервер повернув помилку. Спробуйте пізніше.",
    "requestFailed": "Запит не вдався. Спробуйте ще раз.",
    "network": "Немає з'єднання. Перевірте підключення та спробуйте знову.",
    "dismiss": "Закрити"
  },
  "META": {
    "title": "Book Club — Читацькі клуби України",
    "description": "Приєднуйтесь до книжкових клубів у вашому місті. Обговорення книг, зустрічі читачів, спільноти за інтересами.",
    "ogTitle": "Book Club — Читацькі клуби України",
    "ogDescription": "Приєднуйтесь до книжкових клубів у вашому місті.",
    "twitterTitle": "Book Club — Читацькі клуби України",
    "twitterDescription": "Приєднуйтесь до книжкових клубів у вашому місті. Обговорення книг, зустрічі читачів."
  },
  "COVER_UPLOAD": {
    "upload_image": "📁 Завантажити зображення",
    "uploading": "Завантаження…",
    "enter_url": "🔗 Ввести URL",
    "hide_url": "🔗 Сховати URL",
    "upload_failed": "Не вдалося завантажити. Спробуйте ще раз або скористайтесь URL."
  },
  "SEO": {
